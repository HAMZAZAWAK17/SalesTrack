...
export default function UserList() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      setToast({
        open: true,
        message: error.message || 'Erreur de chargement des utilisateurs. Le serveur est-il actif ?',
        severity: 'error'
      });
    } finally {
      setLoading(false);
      ...
      {toast.open && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-[fadeIn_0.25s_ease-out]">
          <div className={`px-4 py-3 rounded-xl border flex items-center space-x-3.5 shadow-lg ${
            ...
            <button onClick={handleToastClose} className="p-0.5 hover:bg-black/10 rounded-lg cursor-pointer">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
