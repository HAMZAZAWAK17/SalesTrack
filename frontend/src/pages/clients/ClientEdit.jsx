...
export default function ClientEdit() {
  ...
  return (
    <div className={`min-h-screen w-full relative overflow-hidden transition-colors duration-300 font-sans ${
        ...
        <div className="flex items-center gap-4 px-1">
          ...
          <div>
            ...
            <p className={`text-sm mt-0.5 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
              Mettez à jour les informations du compte client ci-dessous.
            </p>
          </div>
        </div>
      ...
      {/* ── Toast Notification ─────────────────────────────────────── */}
      {toast.show && (
        <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-2xl text-sm font-bold transition-all animate-slide-up ${
          toast.type === 'success'
            ? 'bg-emerald-600 text-white shadow-emerald-500/25'
            : 'bg-red-600 text-white shadow-red-500/25'
        }`}>
          <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d={toast.type === 'success'
              ? 'M5 13l4 4L19 7'
